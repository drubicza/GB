...
require "curl.php";
echo "\n\n";
echo "Thank To : Muhammad Ikhsan Aprilyadi\n";
echo "GRATIS GO-FOOD Rp10.000!\n\n";
echo "No HP : ";
...
if (preg_match("/Nomor HP ini tidak valid/",$login_with_phone[1])) {
    ...
    $verify = request("https://api.gojekapi.com/v4/customers/login/verify",
                      '{"client_name":"gojek:cons:android","client_secret":"83415d06-ec4e-11e6-a41b-6c40088ab51e","data":{"otp":"'.$otp.'","otp_token":"'.$otp_token.'"},"grant_type":"otp","scopes":"gojek:customer:transaction gojek:customer:readonly"}',null,$header_login);
    ...
    if (preg_match("/access_token/",$verify[1])) {
        ...
        $order = request("https://api.gojekapi.com/go-promotions/v1/promotions/enrollments",'{"promo_code":"GOFOODOSKM2019"}',null,$header_login);
        $ambil = json_decode($order[1])->data->message;
